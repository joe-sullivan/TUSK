...
public class ChooseTeamActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(_teamsEntered.getString("hockeyTeamList", null) != null){								//read from shared preference to get saved team names if any
			loadTeams();
		}
		if(_teamsEntered.getString("soccerTeamList", null) != null){								//read from shared preference to get saved team names if any
			loadTeams();
		}
	}	
	...
	public void addViews(View v){
		...
		if(!_selectAwayTeam){
			...
			intent.putExtra(StaticFinalVars.CREATE_EDIT, _teams[0]);
	...
	private void editTeam(String newName, String oldName){
		for(int i=0; i<_listOfTeams.getChildCount(); i++){
			if(((TextView)_listOfTeams.getChildAt(i)).getText().toString().equals(oldName)){
				((TextView)_listOfTeams.getChildAt(i)).setText(newName);
			}
		}
	}
		...
		confirmDialog.setPositiveButton("Yes", new DialogInterface.OnClickListener(){							//the positive yes button
			...
			public void onClick(DialogInterface arg0, int arg1) {
				...
				if(_sportType.equals("soccer")){
					...
					intent.putExtra(StaticFinalVars.GAME_INFO, new SoccerGameTime(_teams[0], _teams[1]));
				}
					...
					intent.putExtra(StaticFinalVars.GAME_INFO, new HockeyGameTime(_teams[0], _teams[1]));
				}
				else {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(requestCode == StaticFinalVars.CREATE_TEAM_CODE){
			if(resultCode == Activity.RESULT_OK){
				addNewTeam(data.getStringExtra(StaticFinalVars.TEAM_NAME));
			}
		}
		else if(requestCode == StaticFinalVars.EDIT_TEAM_CODE){
			if(resultCode == Activity.RESULT_OK){
				editTeam(data.getStringExtra(StaticFinalVars.TEAM_NAME), data.getStringExtra(StaticFinalVars.OLD_TEAM_NAME));
			}
		}
	}
}
