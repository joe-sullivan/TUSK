...
public class FootballActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(getLayoutInflater().inflate(R.layout.activity_football, null));
...
//------------------------------------------------------------------------------------------------------------------------------------------
			...
			public void onClick(View v) {
				if(_fieldActive){
					if(_gti.getDownDistance()[0] != 0){
						...
						System.out.println(_gti.getLineOfScrimmage());
					...
					editField(_gti.getLineOfScrimmage(), LINEOFSCRIMMAGE, FIRSTDOWNLINE);
					enableButtons();
					_fieldActive = false;
					
					if(_gti.getDownDistance()[0] == 4){
						fourthDownButtonSet();
					}					
				}
				if(_gti.isReturned()){
	...
	private void editField(int line, String color, String color2){
		...
		_fieldOfPlay.addView(createYard(line, color), line);
		
		_fieldOfPlay.removeViewAt(line-_gti.getDownDistance()[1]);
		_fieldOfPlay.addView(createYard(line-_gti.getDownDistance()[1], color2), line-_gti.getDownDistance()[1]);
	}
