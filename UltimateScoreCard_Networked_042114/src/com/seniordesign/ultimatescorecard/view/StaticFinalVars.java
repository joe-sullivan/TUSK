package com.seniordesign.ultimatescorecard.view;

//names passing data from one intent to new intent (or vice versa)
public class StaticFinalVars {
	
	//main screen to choose team activity
	public static final String SPORT_TYPE = "sportType";
	public static final String LOGIN_STATUS = "login_status";
	
	//create team activity
	public static final String CREATE_EDIT = "createEdit";
	public static final String TEAM_NAME = "teamName";
	public static final String OLD_TEAM_NAME = "oldTeamName";
	public static final int CREATE_TEAM_CODE = 1;
	public static final int EDIT_TEAM_CODE = 2;

	//choose team activity to score keeping interface
	public static final String HOME_TEAM = "home";
	public static final String AWAY_TEAM = "away";
	
	//score keeping interface to statistics page 
	public static final String GAME_TIME = "gameTime";
	public static final String GAME_INFO = "gameInfo";
	public static final String GAME_TEST = "gameTest";

	public static final String GAME_ID = "game_id";
	public static final String GAME_LOG = "gameLog";
	public static final String DISPLAY_TYPE = "display";
	public static final String SHOT_CHART_HOME = "shotChartHome";
	public static final String SHOT_CHART_AWAY = "shotChartAway";
	public static final String SHOT_CHART = "shotChart";
	
	public static final int EMAIL = 3;

	public static final String GAME_CLOCK = "gameclock";
	public static final String GRAPHICS = "graphics";
	
	//score keeping interface to substitutions page
	public static final String SUB_INFO = "subInfo";
	
	//statistics (box) page to individual stats page
	public static final String TEAM_INFO = "teamInfo";
	public static final String PLAYERS_INFO = "playersInfo";
	public static final String PLAYER_NAME = "playerName";
	public static final String HOME_OR_AWAY = "homeOrAway";
	public static final String GAMES = "games";
	public static final String PLAYER = "player";
	public static final String TEAM_INFO2 = "teamInfo2";
	public static final String TEAMS = "teams";
	public static final String AVERAGE = "average";

	public static final String SOCCER_OT_DEFAULT = "15 minutes";
	public static final String SOCCER_DEFAULT = "45 minutes";
	public static final String BASKETBALL_OT_DEFAULT = "5 minutes";
	public static final String BASKETBALL_DEFAULT = "12 minutes";
	public static final String HOCKEY_OT_DEFAULT = "5 minutes";
	public static final String HOCKEY_DEFAULT = "20 minutes";
	public static final String FOOTBALL_OT_DEFAULT = "15 minutes";
	public static final String FOOTBALL_DEFAULT = "15 minutes";
	
	
}
