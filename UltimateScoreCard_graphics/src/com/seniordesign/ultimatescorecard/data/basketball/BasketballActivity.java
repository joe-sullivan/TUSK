 ...
 and the start of the game.*/
	...
	public OnClickListener startGameListener = new OnClickListener(){
		...
		public void onClick(View view) {
			...
			int minuteTime = Integer.parseInt(prefs.getString("perLenBasketball", StaticFinalVars.BASKETBALL_DEFAULT).split(" ")[0]);
		...
		case R.id.nextQuarter:
			...
			if(zeroTime()){
				...
				if(ot == 0){
					...
					int minuteTime = Integer.parseInt(prefs.getString("otLenBasketball", StaticFinalVars.BASKETBALL_OT_DEFAULT).split(" ")[0]);
					_gameClock.restartTimer(minuteTime*60*1000);
					_gameClockView.setOnClickListener(startGameListener);
				}
			}
