...
public class ViewStatsActivity extends Activity{
			...
			builder.setAdapter(arrayAdapter, new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					if(_gameButton.getText().equals(getResources().getString(R.string.choose_game))&&
						...
						buttonEnabler(true,true,false,false,false);
					}
	...
	public OnClickListener sendListener = new OnClickListener(){
		...
		public void onClick(View view) {
			...
			final ArrayList<Players> _homeTeamPlayersPull = (ArrayList<Players>) _db.getPlayersTeam2(_game.gethomeid());
			final ArrayList<Players> _awayTeamPlayersPull = (ArrayList<Players>) _db.getPlayersTeam2(_game.getawayid());
			_home = _db.getTeam(_game.gethomeid());
			_away = _db.getTeam(_game.getawayid());
			...
			*/
			...
			Builder builder = new Builder(ViewStatsActivity.this);
			builder.setTitle("File Selection");
			builder.setMessage("Choose a File Type:");
						...
						try {
							...
							if(_sportButton.getText().equals("Basketball")){
								...
								for(Players p: _awayTeamPlayersPull){
									...
									ast = String.format("%-7s", "   " + stats.getast());
									sog = String.format("%-7s", "   " + stats.getsog());
									ycard = String.format("%-9s", "   " + stats.getycard());
			...
			builder.setNegativeButton("Windows Text", new DialogInterface.OnClickListener() {			
				@Override
				public void onClick(DialogInterface dialog, int which) {				
					fileName = "/" + _away.getabbv() + "vs" + _home.getabbv()+"_"+_game.getDate()+".txt";
					if(isExternalStorageWritable()){
			...
			builder.setNeutralButton("Excel", new DialogInterface.OnClickListener() {			
				@Override
				public void onClick(DialogInterface dialog, int which) {				
